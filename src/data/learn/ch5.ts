...
export const learnCh5: LearnChapter = {
  ...
  beginnerOverview: `ディープラーニングは、コンピュータがデータを使って学習するための技術です。特に、画像を扱う際にはCNN（畳み込みニューラルネットワーク）という手法が使われます。CNNは、画像の中から特徴を見つけ出すのが得意で、例えば猫の写真をたくさん学習させることで、猫を正確に認識できるようになります。また、時系列データを扱うためにはRNN（再帰型ニューラルネットワーク）やその改良版であるLSTM（長短期記憶）を使います。これにより、音声や文章のような順番が重要なデータを処理できます。さらに、Transformerという新しい技術は、全体の情報を一度に処理できるため、より効率的です。これらの技術を学ぶことで、AIの基礎を理解し、実際の問題解決に役立てることができます。`,
  intermediateOverview: `ディープラーニングの要素技術には、画像処理に優れたCNN（畳み込みニューラルネットワーク）、時系列データを扱うRNN・LSTM（長短期記憶）・GRU（ゲート付き再帰ユニット）、そして生成AIの基盤となるTransformerが含まれます。CNNは畳み込み層で局所的特徴を抽出しプーリング層で空間サイズを圧縮します。RNNは長期依存性の問題があり、LSTMのゲート機構が解決策として広く採用されています。

Transformerは、Self-Attention機構により入力全体の関係を並列計算し、RNNの逐次処理の限界を克服した革新的アーキテクチャです。

実務上の重要技術として、過学習対策のドロップアウト、学習安定化のバッチ正規化、深いネットワークを可能にするスキップ結合（ResNet）があります。また転移学習・ファインチューニングにより少ないデータでも高性能なモデルを構築でき、エッジデバイス向けのモデル圧縮（量子化・枝刈り・蒸留）も重要です。G検定では各アーキテクチャの仕組みと適用場面の区別が頻出です。`,
  prerequisites: ['ch4'],
  difficulty: 'advanced',
  sections: [
    {
...
これらの問題を解決するために、畳み込みニューラルネットワーク（CNN）が開発されました。CNNは「局所受容野」という概念を用いて、画像の小さな部分を順次処理し、特徴を抽出します。具体的には、畳み込み層がフィルタ（カーネル）をスライドさせながらエッジやテクスチャなどの局所的な特徴を捉えます。さらに、プーリング層を導入することで、空間サイズを縮小し、位置に対する感度を低下させます。これにより、CNNは少ないパラメータで画像の本質的な特徴を効果的に捉えることができ、計算効率を大幅に向上させることが可能になります。このように、CNNは画像処理において全結合ネットワークの限界を克服し、より高精度な認識を実現しています。`,
      termIds: ['cnn', 'convolutional_layer', 'pooling_layer', 'neural_network', 'fully_connected_layer'],
    },
    {
...
RNNとLSTMは、自然言語処理や音声認識、株価予測など、時間的な文脈を必要とする多くのアプリケーションで広く利用されています。これらの技術は、特にシーケンスデータを扱う際に、その有効性を発揮します。`,
      termIds: ['rnn', 'vanishing_gradient', 'nlp_definition', 'neural_network', 'backpropagation', 'lstm'],
    },
    {
...
ただし、LSTMとGRUはそれぞれ異なる特性を持ち、タスクによって適切な選択が求められます。最近では、Transformerという新しいアーキテクチャが登場し、RNN系のモデルは主流から外れつつありますが、LSTMとGRUは依然として多くの応用で重要な役割を果たしています。`,
      termIds: ['lstm', 'gru', 'neural_network', 'transformer'],
    },
    {
...
また、Transformerは過学習を防ぐためのドロップアウトや、学習の安定化を図るバッチ正規化といった技術も取り入れています。これにより、少ないデータでも高性能なモデルを構築するための転移学習やファインチューニングが可能となります。モデル圧縮技術（量子化、枝刈り、蒸留）も実務において重要であり、エッジデバイスへの展開を容易にします。これらの要素が組み合わさることで、Transformerは現代のAI技術において不可欠な存在となっています。`,
      termIds: ['transformer', 'attention_mechanism', 'self_attention', 'nlp_definition', 'neural_network', 'overfitting'],
    },
    {
...
モデル圧縮は、エッジデバイスへの展開において不可欠な技術です。量子化は、モデルの重みの精度を下げることで軽量化を図り、枝刈り（プルーニング）は重要度の低い重みを削除する手法です。また、知識蒸留は、大きなモデルの知識を小さなモデルに転移することで、計算資源を節約しつつ高い性能を維持します。これらの技術は、特にスマートフォンやIoTデバイスなどのリソースが限られた環境でのAI活用を可能にします。`,
    ...
    },
    {
      ...
      termIds: ['batch_normalization', 'dropout', 'skip_connection', 'neural_network', 'overfitting', 'regularization'],
    },
  ],
  keyTermIds: [
    'cnn',
