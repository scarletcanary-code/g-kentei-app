...
export const learnCh3: LearnChapter = {
  ...
  beginnerOverview: `機械学習とは、コンピュータが大量のデータを使って自分でルールやパターンを学び、予測や分類を行う技術です。主に「教師あり学習」「教師なし学習」「強化学習」の3つの方法があります。例えば、教師あり学習では、スパムメールを判別するために、過去のメールデータに基づいて「スパム」か「通常」かを学習します。教師なし学習では、データの中から自然にグループを見つけることができます。機械学習を使うことで、ビジネスや日常生活での意思決定がより正確になります。`,
  intermediateOverview: `機械学習は、大量のデータから自動的にルールやパターンを学び、予測や分類、判断を行う技術です。主な学習方法には、教師あり学習、教師なし学習、強化学習の3つがあります。教師あり学習は、正解ラベルが付いたデータを用いて学習し、分類（スパムメール判定など）や回帰（価格予測など）に分かれ、決定木・SVM・ランダムフォレストなどのアルゴリズムが使われます。

教師なし学習は、ラベルのないデータから構造を発見する手法で、k-means法によるクラスタリングや主成分分析（PCA）による次元削減が代表例です。強化学習は試行錯誤で報酬を最大化する学習方法で、ゲームAIやロボット制御に応用されています。

モデルの評価には適合率・再現率・F値（分類）やMSE・RMSE（回帰）が用いられます。重要な課題として、訓練データへの過剰適合「過学習」があり、正則化や交差検証で対処します。複数モデルを組み合わせるアンサンブル学習も実務で広く使われます。G検定では、3つの学習パラダイムの違いと評価指標の使い分けが頻出です。`,
  prerequisites: ['ch1'],
...
G検定では、これらの学習パラダイムとそれぞれの代表的なアルゴリズムの理解が求められます。特に、各手法の適用例や利点、欠点を把握することが重要です。`,
      termIds: ['supervised_learning', 'unsupervised_learning', 'reinforcement_learning', 'logistic_regression', 'linear_regression', 'decision_tree'],
    },
    {
...
これらのアルゴリズムは、それぞれ異なる特性を持ち、データの性質やタスクに応じて使い分けることが重要です。`,
      termIds: ['decision_tree', 'svm', 'random_forest', 'ensemble_learning', 'supervised_learning', 'overfitting'],
    },
    {
...
これらの手法を適切に活用することで、機械学習モデルの汎化性能を高め、実務における予測精度を向上させることができます。`,
      termIds: ['overfitting', 'regularization', 'cross_validation', 'hyperparameter', 'loss_function'],
    },
    {
...
勾配ブースティングは、アンサンブル学習の一種で、複数の弱学習器（通常は浅い決定木）を逐次的に追加し、前のモデルの誤差を修正する手法です。このプロセスにより、モデルの精度が向上します。XGBoostやLightGBMなどの実装は、特に競技データ分析において高い精度を発揮し、実務でも広く採用されています。これらの手法は、データの特性に応じて適切に選択されることで、効果的な分類を実現します。`,
      termIds: ['logistic_regression', 'sigmoid_function', 'ensemble_learning', 'supervised_learning', 'decision_tree', 'accountability'],
    },
  ],
  keyTermIds: [
    'supervised_learning',
