  ...
  sections: [
    {
      ...
      intermediateBody: '現代のLLM（大規模言語モデル）はほぼ全てTransformerアーキテクチャを基盤としますが、使用部分により2系統に分類されます。GPTはTransformerのデコーダブロックを積み重ねた自己回帰型モデルです。前のトークン系列を入力として次トークンの確率分布を予測するプロセスを繰り返すことで長文を生成します。ChatGPTはGPTにRLHF（人間フィードバックによる強化学習）を適用し、人間の意図・指示への追従性（alignment）を付与したモデルです。BERTはTransformerのエンコーダのみで構成された双方向文脈理解モデルで、マスク言語モデリングで事前学習されます。文書全体を双方向に参照するため文書分類・質問応答・感情分析など「理解タスク」に優れています。GPT（生成）とBERT（理解）の用途の違いと背景技術はG検定頻出テーマです。',
    ...
    },
    {
      ...
      intermediateBody: '深層生成モデルには主にGAN・VAE・拡散モデルの3アプローチがあります。GAN（生成的敵対ネットワーク）は生成器と識別器が競い合うミニマックスゲームで学習され、高品質な画像生成が可能ですが学習の不安定性（mode collapse等）が課題です。VAE（変分オートエンコーダ）はエンコーダで入力を潜在空間の確率分布に圧縮し、デコーダでサンプリングして再構成する確率的生成モデルです。学習時は「再パラメータ化トリック」（z = μ + σ×ε）でサンプリングを微分可能な変換として扱い、誤差逆伝播を実現します。拡散モデルは前向き過程でデータに段階的にノイズを加え、逆向き過程でノイズ除去を学習して新しいデータを生成します。学習が安定していてGANより多様な生成が可能で、Stable Diffusion等の現代的画像生成AIの基盤です。3モデルの原理と特徴の比較はG検定頻出事項です。',
    ...
    },
    {
      ...
      intermediateBody: 'LLMの実用化において重要な3つの技術を解説します。RAG（Retrieval-Augmented Generation: 検索拡張生成）は、LLMの回答生成時に外部知識ベースから類似文書を検索してコンテキストとして付与する手法です。学習データに含まれない最新情報への対応とハルシネーション（もっともらしい誤情報生成）の抑制に有効です。LoRA（Low-Rank Adaptation）は元のモデルパラメータを固定したまま低ランク行列ペアを追加してファインチューニングする効率的な手法で、全パラメータ更新に比べて大幅に少ない計算コストで実用的な性能を実現します。スケーリング則はモデルパラメータ数・学習データ量・計算量（FLOP）の増加に伴い性能が予測可能に向上するという経験則で、GPT-4等の大規模モデル開発の根拠を提供しています。これら3概念はG検定の重要出題範囲です。',
    ...
    },
    {
      ...
      intermediateBody: 'seq2seq（Sequence-to-Sequence）はTransformer登場前の自然言語処理の主役的アーキテクチャです。RNNベースのエンコーダが入力系列全体を固定長の「文脈ベクトル」に圧縮し、デコーダがその文脈ベクトルから出力系列を逐次生成する仕組みで、機械翻訳・要約・音声認識に広く活用されました。しかし固定長ボトルネックの問題があり、入力が長い文章になると重要な情報が失われる課題がありました。この問題を解決するために開発されたAttention機構はデコーダが各単語を生成する際にエンコーダの各ステップへの注目度を動的に計算し、適切な文脈情報を選択的に参照します。このEncoder（入力理解）とDecoder（出力生成）の分業構造はTransformerに引き継がれ、Encoderのみを使うBERT、Decoderのみを使うGPTという現代LLMの設計思想の基盤となっています。',
