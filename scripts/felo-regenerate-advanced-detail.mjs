#!/usr/bin/env node
/**
 * felo-regenerate-advanced-detail.mjs
 * Felo API を使って用語集の「上級解説」（detail）を再生成するスクリプト
 *
 * 使用方法:
 *   node --env-file=.env scripts/felo-regenerate-advanced-detail.mjs --all [--write] [--dry-run]
 *   node --env-file=.env scripts/felo-regenerate-advanced-detail.mjs --id <termId> [--write] [--dry-run]
 *
 * 環境変数:
...
if (!allFlag && termId === null) {
  ...
  process.stderr.write('使用方法: node scripts/felo-regenerate-advanced-detail.mjs --all\n');
  process.stderr.write('         node scripts/felo-regenerate-advanced-detail.mjs --id <termId>\n');
  process.exit(1);
}
...
- G検定合格を目指す学習者（試験対策段階）
- 専門用語は前提知識として使ってよい
- JDLA シラバスの頻出論点・関連概念・出題されやすい切り口を必ず含める
- 用語間の関連（例: フレーム問題 ↔ シンボルグラウンディング ↔ 強いAI/弱いAI）への言及を推奨
- フィラー文（「重要なスキルとなるでしょう」「ますます重要」「ビジネスや日常生活で活用」「必要なスキル」等）禁止
- 各項目で異なる表現・構成にする（テンプレ化を避ける）
- 文字数目安: 250〜400 字（強制ではない、自然な出力を尊重）

以下の ${batch.length} 件について、それぞれの「上級解説」を JSON 配列で出力してください。
...
以下の用語「${term.term}（${term.termEn}）」の「上級解説」を書いてください。
...
${term.detail || ''}`;
  ...
  if (term.intermediateDetail) {
    prompt += `\n\n【中級解説（参考）】\n${term.intermediateDetail}`;
  }

  prompt += `\n\n出力フォーマット（このフォーマット以外で出力しないこと）:
\`\`\`json
[
  {"id": "${term.id}", "advanced": "<上級解説本文>"}
]
\`\`\``;
  return prompt;
}
...
// バッチ処理（リトライ付き）
// 返却: Map<id, { advanced: string, citations: string[] }>
async function processBatch(batch, batchIndex, totalBatches) {
  ...
  // プロンプトが 2000 字超の場合は単問フォールバックへ
  const prompt = buildBatchPrompt(batch);
  if (prompt.length > 2000) {
    process.stderr.write(`[batch ${batchIndex}/${totalBatches}] prompt length ${prompt.length} > 2000, falling back to single mode\n`);
    return null;
  }
...
// 単問フォールバック処理
// 返却: { advanced: string, citations: string[] } | null
async function processSingleFallback(term, fallbackIndex, fallbackTotal) {
  ...
  if (!item || !item.advanced) {
    // 最初の要素の advanced を使う
    const first = parsed[0];
    if (first && first.advanced) {
      return {
        advanced: first.advanced,
        citations: extractCitations(data),
      };
    }
    process.stderr.write(`[fallback ${fallbackIndex}/${fallbackTotal}] no advanced field\n`);
    return null;
  }
...
// rejected ダンプ
const rejectedPath = join(projectRoot, '../.harness/runs/0030-rejected.json');
...
// バックアップ作成（--write 時のみ）
if (writeFlag) {
  const backupDir = join(projectRoot, '../.harness/runs/0030-backup');
  mkdirSync(backupDir, { recursive: true });
  const backupPath = join(backupDir, 'terms.json.bak');
  copyFileSync(termsPath, backupPath);
  process.stderr.write(`バックアップを作成しました: .harness/runs/0030-backup/terms.json.bak\n`);
}

// 全用語を処理
const resultMap = new Map(); // id -> { advanced, citations }
...
if (writeFlag) {
  const updatedTerms = terms.map((t) => {
    ...
    if (r) {
      const updated = { ...t, detail: r.advanced };
      // source_ref_supplements の merge dedup 1件切り詰め
      if (r.citations && r.citations.length > 0) {
        const existing = Array.isArray(t.source_ref_supplements) ? t.source_ref_supplements : [];
        const merged = [...new Set([...existing, r.citations[0]])].slice(0, 1);
        updated.source_ref_supplements = merged;
      }
      // citations が空の場合は source_ref_supplements を変更しない
      return updated;
    }
    return t;
  });
...
for (const [id, r] of resultMap) {
  ...
  results.push({
    ...
    advancedDetail: r.advanced,
    charCount: r.advanced.length,
